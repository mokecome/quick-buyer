...
-- Note: This uses a system user ID for demo projects
-- In production, these would be linked to real user accounts

-- First, we need to insert without user_id constraint for seed data
-- We'll use a special approach: temporarily disable RLS for seeding

-- Create a system/demo user if needed (for foreign key constraint)
-- This creates a placeholder in auth.users if it doesn't exist
DO $$
DECLARE
  demo_user_id UUID := '00000000-0000-0000-0000-000000000001';
BEGIN
  -- Check if demo user exists, if not we'll use NULL approach
  -- For now, we'll modify the table to allow NULL user_id for seed data
END $$;

-- Temporarily alter table to allow seed data without user
...
-- Add policy for viewing seed projects (those with NULL user_id)
